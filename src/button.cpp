...
{
    pinMode(pin, mode);
    previous_state_ = 0xFF; //read_state(); previously..
...
void Button::loop()
{
    ...
    if (!pressed_active_ && last_change_pressed_ && is_stable) {
        Log.noticeln("# BUTTON TRIGGER PIN %d, debounce_duration_ms: %d last_change_ms: %d", pin_, debounce_duration_ms_, now - down_time_ms_);
        if (pressed_callback != nullptr) {
            pressed_callback ();
        }
        pressed_active_ = true;
    } else if (pressed_active_ && !last_change_pressed_ && is_stable) {
        Log.noticeln("# BUTTON RELEASE TRIGGER PIN %d, debounce_duration_ms: %d last_change_ms: %d", pin_, debounce_duration_ms_, now - down_time_ms_);
        if (released_callback != nullptr) {
            released_callback ();
        }
        pressed_active_ = false;
    }
